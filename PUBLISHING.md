...
The v1.1.0 source and release assets are prepared locally. No remote repository, push, tag publication, or GitHub Release has been created by this audit.
...
6. After CI passes, create the annotated `v1.1.0` tag on the audited commit, push the tag, and create a GitHub Release with `RELEASE_NOTES.md` as the body. Attach `agent-state-governance-v1.1.0.zip` and its `.zip.sha256` sidecar. Verify downloaded asset hashes before announcement.
